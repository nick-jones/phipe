...
class ApplicationConfigTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCreateDefaultStrategies()
    {
        ...
        $this->assertEquals(
            'Phipe\Strategy\Reconnect\SequentialDelayed',
            get_class($this->container['strategies']['reconnect'])
        );
        $this->assertEquals('Phipe\Strategy\Disconnect\Soft', get_class($this->container['strategies']['disconnect']));
        $this->assertEquals(
            'Phipe\Strategy\ActivityDetect\Simple',
            get_class($this->container['strategies']['activity_detect'])
        );
    }
    ...
    public function testCreateDefaultStrategies_NoReconnect()
    {
        ...
        $this->assertEquals(
            'Phipe\Strategy\Disconnect\Expunging',
            get_class($this->container['strategies']['disconnect'])
        );
    }
}